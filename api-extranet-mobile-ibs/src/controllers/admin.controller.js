...
const getUsers = async (req, res) => {
    try {
        ...
        const data = await db.execute('ps_GetUsers', { Nom: nom, Nature: nature });
...
const deleteUser = async (req, res) => {
    try {
        await db.execute('ps_DeleteUser', { Id: req.body.id });
...
const createUserFromClient = async (req, res) => {
    try {
        const data = await db.execute('ps_CreateUserFromClient', { ClientId: req.body.clientId });
...
const createUserFromAdherent = async (req, res) => {
    try {
        const data = await db.execute('ps_CreateUserFromAdherent', { AdherentId: req.body.adherentId });
        res.json({ success: true, data: data[0] });
    } catch (error) { handleError(res, error); }
};
