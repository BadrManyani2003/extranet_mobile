...
const ProfileScreen: React.FC = () => {
  ...
  const hasAdherentRole = userRoles.some(r => r.toLowerCase() === 'adherent');
  ...
  return (
    <Box flex={1} backgroundColor="background">
      ...
      >
        ...
        {/* Personnes à charge (Famille) - For Adherents & Clients */}
        {(hasAdherentRole || hasClientRole) && (
          <Box marginTop="none">
             <Section title={t('PERSONNES À CHARGE')}>
                <TouchableOpacity 
                  style={styles.menuItem} 
                  onPress={() => {
                    // Logic to show family members or navigate to a detail screen
                    Alert.alert(t('Famille'), t('Fonctionnalité de consultation des bénéficiaires.'));
                  }}
                >
                  <Box backgroundColor="successBg" width={32} height={32} borderRadius="round" alignItems="center" justifyContent="center" marginRight="m">
                    <Icon name="people" size={18} color={theme.colors.success} />
                  </Box>
                  <Text variant="bodyMedium" color="text" fontWeight="600" flex={1}>{t('Ma Famille / Bénéficiaires')}</Text>
