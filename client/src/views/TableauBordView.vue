...
const statsCards = computed(() => {
  ...
  return [
    { title: 'Nombre de contrats', value: formatNumber(s.totalPolices || 0), icon: ShieldCheck, color: 'text-slate-900' },
    { title: 'Sinistres en cours', value: formatNumber(s.sinistresEnCours || 0), icon: AlertCircle, color: 'text-orange-600' },
    { title: 'Prime annuelle', value: formatCurrency(s.primeAnnuelle || 0), icon: Activity, color: 'text-emerald-600' },
    { title: 'Primes impayées', value: formatCurrency(s.totalImpayes || 0), icon: CreditCard, color: 'text-red-600' }
  ]
})
...
<template>
  <PageContainer title="Tableau de bord" :subtitle="`Résumé de votre portefeuille pour ${new Date().getFullYear()}`">
    <div v-if="loading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
      ...
      <p class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] animate-pulse">Chargement des données...</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
      <Card class="lg:col-span-2 glass-card p-8 md:p-10 border-none">
        <div class="flex items-center justify-between mb-8">
          <CardTitle class="text-xl font-black text-slate-900 tracking-tight uppercase tracking-widest text-[10px]">Évolution des Primes & Impayés</CardTitle>
      ...
      <Card class="lg:col-span-1 glass-card p-8 md:p-10 border-none">
        <div class="flex items-center justify-between mb-8">
          <CardTitle class="text-xl font-black text-slate-900 tracking-tight uppercase tracking-widest text-[10px]">Répartition par branche</CardTitle>
    ...
    <Card class="glass-card p-8 md:p-10 border-none">
      <div class="flex items-center justify-between mb-8">
        <CardTitle class="text-xl font-black text-slate-900 tracking-tight uppercase tracking-widest text-[10px]">Analyse par branche</CardTitle>
